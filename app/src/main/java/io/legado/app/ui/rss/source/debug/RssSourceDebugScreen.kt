...
fun RssSourceDebugScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.debug_rss_source), color = onSurfaceColor) },
                navigationIcon = {
                ...
                actions = {
                    IconButton(onClick = { onIntent(RssSourceDebugIntent.Clear) }) {
                        Icon(
        ...
        floatingActionButton = {
            FloatingActionButton(
                onClick = {
                    onIntent(
                        if (state.status == BookSourceDebugStatus.Running) RssSourceDebugIntent.Stop
                        else RssSourceDebugIntent.Start
                    )
                },
            ) {
                Icon(
        ...
        LazyColumn(
            ...
            item(key = "controls", contentType = "controls") {
                RssDebugControls(state, onIntent)
            }
                ...
                DebugChipRow {
                    items(BookSourceDebugFilter.entries, key = { it.name }) { filter ->
                        FilterChip(
                            ...
                            onClick = { onIntent(RssSourceDebugIntent.SelectFilter(filter)) },
                        )
                    }
                }
            }
            ...
            DebugChipRow {
                items(RssSourceDebugTarget.entries, key = { it.name }) { target ->
                    FilterChip(
                        ...
                        onClick = { onIntent(RssSourceDebugIntent.SelectTarget(target)) },
                    )
                }
            }
            OutlinedTextField(
                ...
                DebugChipRow {
                    items(visibleExamples, key = { "${it.target}:${it.value}" }) { example ->
                        FilterChip(
                            ...
                            onClick = { onIntent(RssSourceDebugIntent.UseExample(example)) },
                        )
                    }
                }
            }
...
private val RssSourceDebugTarget.titleRes: Int
    get() = when (this) {
        RssSourceDebugTarget.Sort -> R.string.debug_target_sort
        RssSourceDebugTarget.Content -> R.string.debug_target_content
    }

private val RssSourceDebugTarget.hintRes: Int
    get() = when (this) {
        RssSourceDebugTarget.Sort -> R.string.debug_hint_sort
        RssSourceDebugTarget.Content -> R.string.debug_hint_content
    }
