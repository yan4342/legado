...
- **低延迟流式输出**：功能已上线；当前流式接口恢复正常，可实时返回流式响应。
...
- 语音合成的目标文本需填写在 `role` 为 `assistant` 的消息中，**不可**放在 `user` 角色的消息内。
- `user` 角色的消息为可选参数，可以传入指令来调整语音合成的语气与风格，也可以是对话历史（消息内容不会出现在合成的语音中）。使用 `mimo-v2.5-tts-voicedesign` 模型时，为**必填**参数。
...
像给演员写剧本一样，从角色、场景、指导三个维度全方位刻画人物与声线，模型能据此生成更富层次、更具演绎感的语音。
...
> **注意**：如需体验更佳的唱歌风格，**必须**在目标文本最开头添加 `(唱歌)` 标签，格式为：`(唱歌)歌词`。歌词建议采用中文，可获得更优合成效果。标签标识支持等效取值：`唱歌`、`sing`、`singing`，填写任意一项效果一致。
...
支持在文本中任意位置插入 `[音频标签]`。通过 `[音频标签]`，你可以对声音进行细粒度控制，精准调节语气、情绪和表达风格——无论是低声耳语、放声大笑，还是带点小情绪的小吐槽，也可以灵活插入呼吸声、停顿、咳嗽等。语速同样可以灵活调整。
...
无需提供音频文件，只需在角色为 `user` 的消息中添加音色描述文本，即可生成定制化的语音音色。
...
- **合成文本要贴合音色**：`assistant` 消息中的合成文本应与音色描述相匹配，才能获得最佳效果。例如为"温柔治愈系女声"搭配一段晚安独白，而非一段激烈的体育解说。建议使用 LLM 根据你的音色描述自动生成适配的合成文本；在 Studio 页面上，输入音色描述后可直接点击「生成文本」按钮
...
> **注意**：`mimo-v2.5-tts-voicedesign` 的低延迟流式输出功能暂未上线，如有相关需求，请关注近期的功能更新。流式调用接口目前降级为兼容模式，仅在所有推理完成后以流式格式返回一次结果。
...
> **注意**：`mimo-v2.5-tts-voiceclone` 的低延迟流式输出功能暂未上线，如有相关需求，请关注近期的功能更新。流式调用接口目前降级为兼容模式，仅在所有推理完成后以流式格式返回一次结果。
