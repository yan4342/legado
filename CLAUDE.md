...
**MVVM** with AndroidViewModel, coexisting View (XML) and Jetpack Compose screens. **Koin** for DI on Compose screens. **Navigation 3** (`androidx.navigation3` NavDisplay) for Compose-first navigation on the main screen. Legacy Activities/Fragments still use `startActivity<T>()` extensions — no Navigation Compose.
...
- `BaseComposeActivity` — extends `AppCompatActivity`, full Compose Activity base. Handles edge-to-edge, theming, status bar color, background images. Subclasses implement `Content()` composable instead of `onCreate()`. Constructor params: `fullScreen`, `toolBarTheme` (Theme enum), `transparent`, `imageBg`. Provides `LocalStatusBarTransparent` and `LocalStatusBarColor` CompositionLocals. Used by `MainActivity`
- `BaseActivity<VB>` — extends `AppCompatActivity` (legacy XML). Handles theming, system bars, background images, menu tinting. Constructor params: `fullScreen`, `theme` (Theme enum), `toolBarTheme`, `transparent`, `imageBg`. Subclasses implement `onActivityCreated()` instead of `onCreate()`
...
New Compose screens use Koin for ViewModel injection. Module definitions in `di/AppModule.kt`:
...
ViewModels registered in Koin: `SearchViewModel`, `ExploreShowViewModel`, `MainViewModel`, `BookshelfViewModel`, `ExploreViewModel`, `RssViewModel`, `DictViewModel`, `CrashViewModel`.

Legacy View-based Activities/Fragments still use `viewModels()` delegates — no Koin for those.

### Coil 3 Image Loading (`help/coil/`)

Image loading via Coil 3, initialized in `App.onCreate()` through `CoilInitializer.init()`. Configuration in `CoilInitializer`:
- `LegadoFetcher` — OkHttp-based network fetcher (reuses app's OkHttp client)
- SVG decoder via service-loader auto-registration
- Disk cache: 1GB in `coil_image_cache` directory
- Logger: `DebugLogger()` in debug builds (currently commented out)
- Replaces the previous Glide-based `LegadoGlideModule`

### Main Screen: Navigation 3 Compose Architecture (`ui/main/`)

The main screen has been fully migrated from `ViewPager` + Fragment tabs to Navigation 3 Compose:

```
MainActivity (BaseComposeActivity)
└── Content() → SharedTransitionLayout → MainNavHost()
    ├── entry<MainRouteHome> → BottomNavScreen (Scaffold + NavigationBar + HorizontalPager)
    │   ├── BookshelfTab (Compose, with own NavDisplay for book info drill-down)
    │   ├── ExploreScreen (Compose)
    │   ├── RssScreen (Compose)
    │   └── MyScreen (Compose)
    ├── entry<MainRouteSearch> → SearchScreen
    ├── entry<MainRouteBookInfo> → BookInfoRouteScreen
    ├── entry<MainRouteReadRecord> → ReadRecordRoute
    ├── entry<MainRouteAbout> → MyAboutRoute
    └── ... (settings routes, explore show, etc.)
```

**Key files:**
- `MainNavGraph.kt` — `MainNavHost` composable. Single `NavDisplay` with all routes via `entryProvider { entry<Route> { ... } }`
- `MainNavKey.kt` — Sealed interface `MainRoute` with `@Serializable` data objects/classes for every destination. Also `MainRouteConst` with path strings for deep-link intents
- `MainNavigator.kt` — Navigation logic: `navigateToRoute(backStack, route)`, `navigateBack()`, back-stack mutation rules
- `MainIntent.kt` — External entry points: `createHomeIntent()`, `createSearchIntent()`, etc. for launching MainActivity with a specific route
- `BottomNavScreen.kt` — `Scaffold` with `NavigationBar` (Material 3) + `HorizontalPager` for 2-4 tabs (Bookshelf, optionally Explore/RSS, My)
- `MainDestination.kt` — Enum for bottom nav tabs

**Navigation 3 conventions:**
- Routes are `@Serializable` sealed interface members — serializable by the navigation framework for persistence
- `rememberNavBackStack(MainRouteHome)` creates the back stack
- `rememberSaveableStateHolderNavEntryDecorator()` + `rememberViewModelStoreNavEntryDecorator()` for state/ViewModel scoping per entry
- `SinglePaneSceneStrategy()` for single-pane layout
- Transition specs use `slideIntoContainer` + `fadeIn` / `scaleOut` + `fadeOut` via `NavDisplay.transitionSpec` / `popTransitionSpec`
- Predictive back via `predictivePopTransitionSpec`
- External route navigation: `MainActivity` exposes `navigateToSearch()` backed by a route setter callback
- `MainNavigator.navigateBack()` handles `Activity.finish()` for root-level back on some routes (settings sub-routes that launched as Activities)

**ViewModel creation with fragment=null:**

When `MainNavGraph` creates route composables (MyOtherConfigRoute, MyBackupConfigRoute, AiDictRuleRoute), it passes `fragment = null`. These ViewModels are `AndroidViewModel` subclasses. Use `AndroidViewModelFactory` when fragment is null:
...
Do NOT use `LocalViewModelStoreOwner.current!!` for `AndroidViewModel` subclasses — it lacks `APPLICATION_KEY`.

### MVI/UDF Pattern

New Compose screens follow a lightweight MVI/UDF pattern:
...
// Screen composable receives viewModel, calls viewModel.onIntent(...)
@Composable
fun SearchScreen(viewModel: SearchViewModel, onBack: () -> Unit, ...)
```

ViewModels using this pattern: `SearchViewModel`, `ExploreShowViewModel`, `ExploreViewModel`, `RssViewModel`, `ReadRecordViewModel`.
...
### Entry points
- `App.kt` — Application class (Cronet, Rhino, notifications, Koin `startKoin`, theme init)
- `ui/welcome/WelcomeActivity.kt` — LAUNCHER activity
- `ui/main/MainActivity.kt` — Main screen: `BaseComposeActivity` with Navigation 3 Compose bottom nav (Bookshelf, Explore, RSS, My)
...
Compose is the forward path: new screens (main tabs, BookInfo, ReadRecord, Search) are Compose; legacy XML still handles reading, some settings, and some dialogs.

**Entry point — full Compose Activity (`BaseComposeActivity`)**:
```kotlin
...
**Entry point — Compose Fragment (legacy, still used by some screens)**:
```kotlin
class ConfigComposeFragment : BaseFragment(0) {
...
**Navigation 3 route composables**: Stateless composables + ViewModel via `koinViewModel()`:
```kotlin
// In MainNavGraph entry block:
entry<MainRouteSearch> { route ->
    val searchViewModel = koinViewModel<SearchViewModel>()
    LaunchedEffect(route.key) {
        searchViewModel.onIntent(SearchIntent.Initialize(...))
    }
    SearchScreen(viewModel = searchViewModel, onBack = onNavigateBack, ...)
}
```

**AndroidViewModel route composables (non-Koin)**: When NOT using Koin (e.g. ConfigViewModel, AiDictRuleViewModel), pass `fragment = null` and use `AndroidViewModelFactory`:
```kotlin
val application = LocalContext.current.applicationContext as Application
...
- **Main activity**: Compose `NavDisplay` with `NavigationBar` (Material 3) + `HorizontalPager`. Bottom nav has 2-4 tabs based on `AppConfig.showDiscovery`/`showRSS`. No TitleBar
